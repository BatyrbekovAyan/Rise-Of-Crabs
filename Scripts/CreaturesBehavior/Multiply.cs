...
public class Multiply : MonoBehaviour, ICreatureBody
{
    ...
    public void Start()
    {
        number = (int)Mathf.Pow(2f, Random.Range(1, 1 - (Levels.level - 1) % Creatures.sameLevelsNumber + Mathf.FloorToInt(Mathf.Sqrt(Creatures.levelRow - 1)) - Mathf.FloorToInt((Levels.level - 1) / 48)));
        ...
        totalLife = 20 + Creatures.levelRow / 2;
        currentLife = totalLife;
        speed = 0.2f * Mathf.Sqrt(50 + 0.1f * Levels.level);
        endPointPresition = Random.Range(0.001f, 0.1f);
        horizontalDistanceMin = 2f;
        horizontalDistanceMax = 3f;
        verticalDistanceMin = 0.1f;
        verticalDistanceMax = 0.5f;
        ...
        Rune = transform.GetChild(2);

        transform.parent.GetComponent<Creature>().totalLife = 5 + Creatures.levelRow / 16;
    ...
    public void FixedUpdate()
    {
        if (moveEnabled)
        {
            ...
            {
                transform.position = Vector2.LerpUnclamped(transform.position, newPosition, speed * Time.deltaTime);
    ...
    public IEnumerator Shot()
    {
        ...
        transform.parent.GetComponent<Creature>().GivePoints(20);
    ...
    private void Turn()
    {
        ...
        Rune.GetComponent<RectTransform>().localScale = new Vector3(-Rune.GetComponent<RectTransform>().localScale.x, 1, 1);
        Rune.GetComponent<RectTransform>().rotation = Quaternion.Euler(0, Rune.GetComponent<RectTransform>().rotation.eulerAngles.y, -Rune.GetComponent<RectTransform>().rotation.eulerAngles.z);
    }
